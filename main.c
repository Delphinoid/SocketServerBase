...
void ssHandleDisconnectTCP(socketServer *server, socketDetails *details){
	...
	inet_ntop(details->address.ss_family,
	          ...
	          &IP[0],
	          sizeof(IP));
	printf("Closing TCP connection with %s:%u (socket #%d).\n", IP, ((struct sockaddr_in *)&details->address)->sin_port, details->id);
...
void ssHandleConnectUDP(socketServer *server, socketDetails *details){
	...
	inet_ntop(details->address.ss_family,
	          ...
	          &IP[0],
	          sizeof(IP));
	printf("Accepted UDP connection from %s:%u (socket #%d).\n", IP, ((struct sockaddr_in *)&details->address)->sin_port, details->id);
}
...
void ssHandleBufferUDP(const socketServer *server, socketDetails *details){
	...
	inet_ntop(details->address.ss_family,
	          ...
	          &IP[0],
	          sizeof(IP));
	printf("Data received over UDP from %s:%u: %s\n", IP, ((struct sockaddr_in *)&details->address)->sin_port, details->lastBuffer);
...
void ssHandleDisconnectUDP(socketServer *server, socketDetails *details){
	...
	inet_ntop(details->address.ss_family,
	          ...
	          &IP[0],
	          sizeof(IP));
	printf("Closing UDP connection with %s:%u (socket #%d).\n", IP, ((struct sockaddr_in *)&details->address)->sin_port, details->id);
	scRemoveSocket(&server->connectionHandler, details);
}
